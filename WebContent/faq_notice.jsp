...
<%
	Vector<SubmitBean> list=submitMgr.getSubmitList(1);
...
<table class="table">
  ...
  <a href="faq.jsp?page=notice&index=<%if(idx-3<0) {%><%=idx%><%}else{%><%=idx-3%><%} %>" 
	       class="btn btn-default" style="height:50px;"><h4>이전</h4></a>
  <a href="faq.jsp?page=notice&index=<%if(idx+3>list.size()-1){%><%=idx%><%}else{%><%=idx+3%><%}%>" 
